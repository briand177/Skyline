...
export async function initFCI() {
    ...
    try {
        ...
        renderFciTable();
        renderFciPortfolio(); 
    } catch (error) { console.error("Error crítico FCIs:", error); }
}
...
function parseFciData(data) {
    ...
    return fondosValidos.map(item => ({
        symbol: item.fondo, c: parseFloat(item.vcp) || 0, tipo: nombresTipos[item.tipoFondo] || 'Fondo Común', fecha: item.fecha || 'Sin fecha'
...
function renderFciTable() {
    ...
    if (searchTerm !== "") filtered = fciData.filter(item => matchSearch(`${item.symbol} ${item.tipo}`, searchTerm));
    const dataToRender = searchTerm === "" ? filtered.slice(0, 100) : filtered;

    fciResults.innerHTML = dataToRender.map(item => `
        <tr class="fci-row">
            <td><strong>${item.symbol}</strong></td>
            <td>$ ${formatPrecio(item.c)}</td>
...
document.getElementById("btnOpenFciTxModal").addEventListener("click", () => {
    fciModal.classList.add("active"); document.body.style.overflow = "hidden";
    if (!editingFciId) { const today = new Date(); document.getElementById("fciTxDate").value = new Date(today.getTime() - (today.getTimezoneOffset() * 60000)).toISOString().split('T')[0]; }
});
...
function closeFciModal() {
    fciModal.classList.remove("active"); document.body.style.overflow = "auto"; document.getElementById("fciTransactionForm").reset();
    document.getElementById("fciTickerSuggestions").style.display = "none"; editingFciId = null;
    document.getElementById("fciFormTitle").innerText = "Operación de FCI"; document.getElementById("btnSubmitFciTx").innerText = "Guardar FCI";
...
fciTickerInput.addEventListener("input", () => {
    const val = fciTickerInput.value.toLowerCase(); fciTickerSuggestions.innerHTML = "";
    ...
    if (matches.length > 0) {
        ...
        matches.forEach(match => { const li = document.createElement("li"); li.textContent = match.symbol; li.addEventListener("click", () => { fciTickerInput.value = match.symbol; fciTickerSuggestions.style.display = "none"; }); fciTickerSuggestions.appendChild(li); });
...
document.getElementById("fciTransactionForm").addEventListener("submit", (e) => {
    ...
    const newTx = {
        ...
        ticker: document.getElementById("fciTxTicker").value.trim(), type: document.getElementById("fciTxType").value,
        currency: document.getElementById("fciTxCurrency").value, qty: Math.abs(parseFloat(document.getElementById("fciTxQty").value)),
        price: Math.abs(parseFloat(document.getElementById("fciTxPrice").value)), date: document.getElementById("fciTxDate").value
    };
    if (editingFciId) { const idx = fciTransactions.findIndex(t => String(t.id) === String(editingFciId)); if (idx !== -1) fciTransactions[idx] = newTx; } 
    else { fciTransactions.push(newTx); }
    localStorage.setItem('skyline_fci_txs', JSON.stringify(fciTransactions));
    closeFciModal(); renderFciHistory(); renderFciPortfolio();
});

export function renderFciPortfolio() {
    const holdings = {};
    let actInvARS = 0, actInvUSD = 0, actCurARS = 0, actCurUSD = 0;
    let clsInvARS = 0, clsInvUSD = 0, clsProARS = 0, clsProUSD = 0;
    let fciHoldingsArr = []; 
    ...
    sorted.forEach(tx => {
        ...
        if (tx.type === "buy") { holdings[tx.ticker].qty += tx.qty; holdings[tx.ticker].invARS += costARS; holdings[tx.ticker].invUSD += costUSD; holdings[tx.ticker].invNative += nativeCost; } 
        else if (tx.type === "sell" && holdings[tx.ticker].qty > 0) {
            const soldQty = Math.min(tx.qty, holdings[tx.ticker].qty);
            const avgARS = holdings[tx.ticker].invARS / holdings[tx.ticker].qty; const avgUSD = holdings[tx.ticker].invUSD / holdings[tx.ticker].qty; const avgNative = holdings[tx.ticker].invNative / holdings[tx.ticker].qty;
            clsInvARS += (avgARS * soldQty); clsInvUSD += (avgUSD * soldQty);
            const nativeProceeds = (soldQty / 1000) * tx.price;
            clsProARS += tx.currency === 'USD' ? nativeProceeds * txMep : nativeProceeds; clsProUSD += tx.currency === 'USD' ? nativeProceeds : nativeProceeds / txMep;
            holdings[tx.ticker].qty -= soldQty; holdings[tx.ticker].invARS -= (avgARS * soldQty); holdings[tx.ticker].invUSD -= (avgUSD * soldQty); holdings[tx.ticker].invNative -= (avgNative * soldQty);
        }
    });
    ...
    for (let t in holdings) {
        ...
        actInvARS += h.invARS; actInvUSD += h.invUSD; actCurARS += currentValARS; actCurUSD += currentValUSD;
    ...
    const dActInv = isUSD ? actInvUSD : actInvARS; const dActCur = isUSD ? actCurUSD : actCurARS; const dActPnl = dActCur - dActInv;
    document.getElementById("fciActiveInvested").innerText = obfuscate(sym + formatMontoEntero(dActInv)); document.getElementById("fciActiveCurrent").innerText = obfuscate(sym + formatMontoEntero(dActCur)); document.getElementById("fciActivePNL").innerText = obfuscate(`${sym}${formatMontoEntero(dActPnl)} (${(dActInv > 0 ? (dActPnl / dActInv * 100) : 0).toFixed(2)}%)`); document.getElementById("fciActivePNL").className = dActPnl >= 0 ? "positive" : "negative";

    const dClsInv = isUSD ? clsInvUSD : clsInvARS; const dClsCur = isUSD ? clsProUSD : clsProARS; const dClsPnl = dClsCur - dClsInv;
    document.getElementById("fciClosedInvested").innerText = obfuscate(sym + formatMontoEntero(dClsInv)); document.getElementById("fciClosedCurrent").innerText = obfuscate(sym + formatMontoEntero(dClsCur)); document.getElementById("fciClosedPNL").innerText = obfuscate(`${sym}${formatMontoEntero(dClsPnl)} (${(dClsInv > 0 ? (dClsPnl / dClsInv * 100) : 0).toFixed(2)}%)`); document.getElementById("fciClosedPNL").className = dClsPnl >= 0 ? "positive" : "negative";

    const dTotInv = dActInv + dClsInv; const dTotCur = dActCur + dClsCur; const dTotPnl = dTotCur - dTotInv;
    document.getElementById("fciTotalInvested").innerText = obfuscate(sym + formatMontoEntero(dTotInv)); document.getElementById("fciTotalCurrent").innerText = obfuscate(sym + formatMontoEntero(dTotCur)); document.getElementById("fciTotalPNL").innerText = obfuscate(`${sym}${formatMontoEntero(dTotPnl)} (${(dTotInv > 0 ? (dTotPnl / dTotInv * 100) : 0).toFixed(2)}%)`); document.getElementById("fciTotalPNL").className = dTotPnl >= 0 ? "positive" : "negative";
...
window.editarFciTx = function(id) {
    ...
    if(tx) {
        editingFciId = tx.id; document.getElementById("fciTxTicker").value = tx.ticker; document.getElementById("fciTxType").value = tx.type; document.getElementById("fciTxCurrency").value = tx.currency || "ARS"; document.getElementById("fciTxQty").value = tx.qty; document.getElementById("fciTxPrice").value = tx.price; document.getElementById("fciTxDate").value = tx.date; document.getElementById("fciFormTitle").innerText = "Editando Operación FCI"; document.getElementById("btnSubmitFciTx").innerText = "Actualizar"; document.getElementById("btnCancelFciEdit").style.display = "block";
        const fciModal = document.getElementById("fciTxModal"); fciModal.classList.add("active"); document.body.style.overflow = "hidden";
    }
}

window.eliminarFciTx = function(id) { if(confirm("¿Eliminar operación de FCI?")) { fciTransactions = fciTransactions.filter(t => String(t.id) !== String(id)); localStorage.setItem('skyline_fci_txs', JSON.stringify(fciTransactions)); renderFciHistory(); renderFciPortfolio(); } }

const btnExportFci = document.getElementById("btnExportFci");
if(btnExportFci) { btnExportFci.addEventListener("click", () => { if (fciTransactions.length === 0) { alert("No hay transacciones de FCI para exportar."); return; } let csvContent = "\uFEFFid;ticker;type;currency;qty;price;date\n"; fciTransactions.forEach(t => { const pTxt = t.price.toString().replace('.', ','); const qTxt = t.qty.toString().replace('.', ','); csvContent += `${t.id};${t.ticker};${t.type};${t.currency};${qTxt};${pTxt};${t.date}\n`; }); const hoy = new Date(); const fechaTxt = hoy.toISOString().split('T')[0]; const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.setAttribute("href", url); link.setAttribute("download", `skyline_fci_${fechaTxt}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link); }); }

const btnImportFci = document.getElementById("btnImportFci"); const fileImportFci = document.getElementById("fileImportFci");
        ...
        reader.onload = function(event) {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (cols.length >= 7) {
                    ...
                    importedTxs.push({ id: cols[0].trim(), ticker: cols[1].trim(), type: cols[2].trim().toLowerCase().includes("sell") ? "sell" : "buy", currency: cols[3].trim().toUpperCase() === "USD" ? "USD" : "ARS", qty: Math.abs(parseFloat(cols[4].replace(/,/g, '.'))), price: parseFloat(cols[5].replace(/,/g, '.')), date: parsedDate });
                }
            }
            if (importedTxs.length > 0) {
                if (confirm(`Se leyeron ${importedTxs.length} operaciones de FCI.\n\n¿REEMPLAZAR tu historial actual de fondos?`)) { fciTransactions = importedTxs; } 
                else { const existingIds = new Set(fciTransactions.map(t => String(t.id))); importedTxs.forEach(t => { if (existingIds.has(String(t.id))) t.id = Date.now() + Math.random(); fciTransactions.push(t); }); }
                localStorage.setItem('skyline_fci_txs', JSON.stringify(fciTransactions)); renderFciHistory(); renderFciPortfolio(); alert("¡Fondos importados con éxito!");
            } else alert("Revisá el formato del archivo CSV."); fileImportFci.value = ""; 
